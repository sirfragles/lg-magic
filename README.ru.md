...
```
 ...
 └──────────────┘                                 │ IMU (никогда не
                   ...
                   ▼                                          ▼
            ОДНА виртуальная мышь                    CLI `lg-magic`
            + клавиатура                     (libc/libm, свой D-Bus клиент)
```
...
- **Демон добавляет остальное.** `lg-magicd` захватывает evdev
  ...
  виртуальную клавиатуру**. Когда демон останавливается — включая
  падения — захват снимается вместе с его файловыми дескрипторами, и
  пульт возвращается к сырому вводу от ядра.
...
- sd-bus интерфейс (`org.lgmagic`, `/org/lgmagic/Manager`) с проверкой
  polkit на каждом изменяющем методе; evdev-устройство IMU никогда не
  захватывается, поэтому `lg-magic imu` работает параллельно
...
7. **Blob прошивки** — `lg_magic_calib_XX_XX_XX_XX_XX_XX.bin` для
   Bluetooth MAC вашего пульта (+ запасной `lg_magic_calib.bin`) в
   `/lib/firmware/`
8. **Состояние демона** (в режиме демона) — записывает
   ...
   (`systemctl enable --now`, по возможности)
