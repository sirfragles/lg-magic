...
def parse_fd(data):
    ...
    points_of_interest = [
        int.from_bytes(data[i : i + 2], "big", signed=True) for i in range(5, 17, 2)
    ]

    # Print neatly
    print(
        f"{'Counter':>8}  {'Const':>6}  {'POI1':>6}  {'POI2':>6}  {'POI3':>6}  {'ACCEL_X':>6}  {'ACCEL_Y':>6}  {'ACCEL_Z':>6}"
    )
    print(f"{counter:8}  {hex(const_fd00):>6}  " + "  ".join(f"{p:6}" for p in points_of_interest))
